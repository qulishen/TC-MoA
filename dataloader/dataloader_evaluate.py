...
class EvaluateDataSet(Dataset):
                ...
                for path in os.listdir(rgb_dir):
                    ...
                    if os.path.isdir(class_path):
                        for file in os.listdir(class_path):
                            ...
                            if "1." in file or "_b." in file:        #B是过曝 oe    或者远焦far
                                rgb_list.append(file_path)
                               
                            if "0." in file or "_A." in file:        #A是低曝光 ue  或者近焦next
    ...
    def save_img_NewLoader(self,img_tensor_list,path,info_list):
        ...
        # 进一步做简单的0.5/99.5分位拉伸，缓解少量极端值导致的低对比度
        flat = img_norm.flatten()
        p_low = torch.quantile(flat, 0.005)
        p_high = torch.quantile(flat, 0.995)
        if (p_high - p_low) > 1e-6:
            img_norm = img_norm.clamp(p_low, p_high)
            img_norm = (img_norm - p_low) / (p_high - p_low)
        img_np = img_norm.numpy().astype(np.float32)
        # 选择 scipy.misc.imsave 保存，若不可用则回退 imageio
        # 按目录现有文件数量顺序命名：1.ext、2.ext...
        ext = os.path.splitext(info_list["name"])[1] or ".png"
        existing_files = [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]
        next_idx = len(existing_files) + 1
        img_name = f"{next_idx}{ext}"
        img_path = os.path.join(path, img_name)
        save_success = False
        if scipy_imsave is not None and not save_success:
            try:
                print("use scipy.misc.imsave")
                scipy_imsave(img_path, img_np)
                save_success = True
            except Exception as e:
                print("scipy_imsave_failed", e)
        if (not save_success) and imsave is not None:
            try:
                imsave(img_path, (img_np * 255.0).astype(np.uint8))
                save_success = True
            except Exception as e:
                print("imageio_imsave_failed", e)
        if not save_success:
            try:
                from PIL import Image
                pil_img = Image.fromarray((img_np * 255.0).clip(0,255).astype(np.uint8))
                pil_img.save(img_path)
                print("use PIL.Image.save")
                save_success = True
            except Exception as e:
                print("PIL_save_failed", e)
        # 打印保存前的统计，便于排查黑图
        print("save_stats",
              "path:", img_path,
              "min:", float(img_np.min()),
              "max:", float(img_np.max()),
